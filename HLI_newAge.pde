...
TODO URGENT
------------
.Map arrays -> parameters map types e.g island, indoor situation, etc
    -> Consider how different things will be placed on tiles and their functions called (look at new edition)
    -> (Floors) and (things on tiles)
.Map generation -> staged
.Followers -> Inspect tiles
.Create textFormatted thing
.Select items from catalogue list
.Home screen
.Options menu + Hotkeys menu
.Preset
.Inventory screen
.Shop screen -> tradingOutpost, import from other places on world map
.Multi-Tile structures + ghosts
.Animation
.Modes e.g exploration, building, piping, crops
.Crop growth
.Reworked pipes
.Day night filtering
.Spawning entities -> Hold drinks, have speech
    -> Path finding
.Events menu on right (like a scroll)
.Factions
.World map
.Travelling to your inn
.Travelling to other inns
.Items on floor
.Items in container
.Items -> Have linked text and quantity
.Hovered tiles and inventory spaces
.Overlays -> e.g sleep, weather, urgency (faster stuff when lots of people and lots going on)
.Certain tiles (e.g Vats) can hold fluid for beer
.Buttons for specific spaces
